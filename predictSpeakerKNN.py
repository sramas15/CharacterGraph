...
def train_KNN(
		...
		priorlims=np.arange(3, 15, 2)): # ks to explore
	...
	searchmod = KNeighborsClassifier()
	# Parameters to grid-search over:
	parameters = {'n_neighbors':priorlims, 'weights':['uniform','distance'],'algorithm':['ball_tree','kd_tree','brute']}
	...
	mod = KNeighborsClassifier(n_neighbors=params['n_neighbors'], weights=params['weights'], algorithm=params['algorithm'])
	##### ASSESSMENT
...
if __name__ == "__main__":
	...
	for i in xrange(36):
		...
		print
		# train model
		model = train_KNN(feats = filteredFeatures, labels = filteredLabels, cv = 10)
