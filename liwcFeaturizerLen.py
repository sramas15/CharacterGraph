	...
	with open(filename, 'r') as f:
		while True:
			...
			while True:
				...
				speech += " " + line
			for pair in pairs:
				speeches.append((pair, speech.strip()))
	return speeches
...
def writeLIWCFeatures(indFilename, mergedFilename, features, dictionary):
	...
	indWriter = csv.writer(outputInd, delimiter=',', quoting=csv.QUOTE_MINIMAL)
	outputMerged = open(mergedFilename, 'wb')
	mergedWriter = csv.writer(outputMerged, delimiter=',', quoting=csv.QUOTE_MINIMAL)
	# write the headers
...
if __name__ == "__main__":
	...
	for i in xrange(36):
		speeches = readTriples('triples2/triples-' + str(i) + '.txt')
		features = featurizeSpeeches(dictionary, speeches)
		writeLIWCFeatures('Triples-LIWC2/' + str(i) + '-individual-len.csv', 'Triples-LIWC2/' + str(i) + '-merged-len.csv', features, dictionary)

	# combine files for each play into an aggregate
	indComb = open('Triples-LIWC2/individual-combine-len.csv', 'wb')
	indWriter = csv.writer(indComb, delimiter=',', quoting=csv.QUOTE_MINIMAL)
	mergedComb = open('Triples-LIWC2/merged-combine-len.csv', 'wb')
	mergedWriter = csv.writer(mergedComb, delimiter=',', quoting=csv.QUOTE_MINIMAL)
	...
	for i in xrange(36):
		with open('Triples-LIWC2/' + str(i) + '-individual-len.csv', 'r') as indFile:
			indFile.readline()
			lines = indFile.readlines()
			for line in lines:
				indComb.write(line)
		with open('Triples-LIWC2/' + str(i) + '-merged-len.csv', 'r') as mergedFile:
			mergedFile.readline()
